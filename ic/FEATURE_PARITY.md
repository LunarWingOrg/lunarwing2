...
| Skill demotion & pruning (B-2) | ✅ | ✅ | Confidence-based demotion (inline in `record_usage` + activation exclusion in `handle_list_skills`), `deprecated_at`/`archived_at` flags (not trust tiers), `__propose_skill_prune__` host fn, skill-maintenance sweep mission, archive (soft-delete) via unified proposals surface |
| Cross-agent skill sharing (B-3) | ✅ | ✅ | ClawHub publish (`POST /api/v1/skills` multipart + Bearer), publish-eligibility gate (Trusted + proven), client-side LeakDetector pre-flight, registry provenance via `.registry.json` sidecar + v1→v2 migration stamping, leak-scan on pull, version-check/`pending_update` proposal path, `POST /api/skills/publish/{doc_id}` + GUI Publish button |
