...
| Telegram | ✅ | ❌ | - | Removed from LunarWing; proprietary channels are intentionally unsupported |
...
| Per-channel media limits | ✅ | 🚧 | Caption support plus `mediaMaxMb` enforcement for proprietary channels is not carried |
...
| Self-improving skills (B-1) | ✅ | ✅ | Confidence-threshold patch proposals (propose→approve), patch history with metrics snapshots, epoch-reset on apply, `__propose_skill_patch__` host fn, `/api/skills/patches` + proposals API/GUI |
| Skill demotion & pruning (B-2) | ✅ | ✅ | Confidence-based demotion (inline in `record_usage` + activation exclusion in `handle_list_skills`), typed automatic-demotion provenance, configurable quarantine before user-approved pruning, `deprecated_at`/`archived_at` flags (not trust tiers), scoped/gated `__propose_skill_prune__`, archive (soft-delete) via unified proposals surface |
| Cross-agent skill sharing (B-3) | ✅ | ✅ | ClawHub publish (`POST /api/v1/skills` multipart + Bearer) with parseable manifest and local provenance stamping, publish-eligibility/leak gates, exact-slug install provenance, gated daily version detection, hash-bound `pending_update`, validated prompt/activation/snippet replacement on approval, registry contract tests, publish API + GUI action |
...
- ❌ Plugin registry
- 🚧 Streaming (Engine V2 gateway block-level text streaming via SSE; XMPP/DarkIRC/WeeChat opt-in final-response delivery; tool-level + Z.AI tool_stream deferred)
