...
/// XMPP polling offsets) between poll ticks without requiring a
...
mod tests {
    ...
    fn test_channel_name() {
        let caps = ChannelCapabilities::for_channel("xmpp");
        let state = ChannelHostState::new("xmpp", caps);

        assert_eq!(state.channel_name(), "xmpp");
    }
    ...
    fn test_channel_workspace_store_commit_and_read() {
        ...
        // Initially empty
        assert!(store.read("channels/xmpp/offset").is_none());

        // Commit some writes
        let writes = vec![
            PendingWorkspaceWrite {
                path: "channels/xmpp/offset".to_string(),
                content: "103".to_string(),
            },
            PendingWorkspaceWrite {
                path: "channels/xmpp/state.json".to_string(),
        ...
        // Should be readable
        assert_eq!(store.read("channels/xmpp/offset"), Some("103".to_string()));
        assert_eq!(
            store.read("channels/xmpp/state.json"),
        ...
        assert_eq!(store.read("channels/xmpp/offset"), Some("200".to_string()));
    ...
    fn test_workspace_write_then_read_round_trip() {
        ...
        let caps = ChannelCapabilities::for_channel("xmpp");
        let mut state = ChannelHostState::new("xmpp", caps);
        ...
        let mut caps2 = ChannelCapabilities::for_channel("xmpp");
        ...
        let state2 = ChannelHostState::new("xmpp", caps2);

        // workspace_read prefixes path with "channels/xmpp/" before delegating.
    ...
    fn test_channels_have_isolated_namespaces() {
        ...
        // XMPP writes "offset" = "100".
        let caps_tg = ChannelCapabilities::for_channel("xmpp");
        let mut state_tg = ChannelHostState::new("xmpp", caps_tg);
        state_tg
        ...
        let mut caps_tg_read = ChannelCapabilities::for_channel("xmpp");
        caps_tg_read.tool_capabilities.workspace_read = Some(WorkspaceCapability {
            allowed_prefixes: vec![],
            reader: Some(Arc::clone(&store) as Arc<dyn WorkspaceReader>),
        });
        let tg_reader = ChannelHostState::new("xmpp", caps_tg_read);
        assert_eq!(
