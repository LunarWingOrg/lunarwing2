...
mod tests {
    ...
    fn test_safe_channel_key() {
        assert_eq!(safe_channel_key("xmpp").unwrap(), "xmpp");
        assert_eq!(safe_channel_key("XMPP").unwrap(), "xmpp");
    ...
    fn test_list_pending_empty() {
        ...
        let requests = store.list_pending("xmpp").unwrap();
    ...
    fn test_upsert_request_creates_new() {
        let (store, _) = test_store();
        let result = store
            .upsert_request("xmpp", "user123", Some(serde_json::json!({"chat_id": 456})))
            .unwrap();
    ...
    fn test_upsert_request_updates_existing() {
        let (store, _) = test_store();
        let r1 = store.upsert_request("xmpp", "user123", None).unwrap();
        assert!(r1.created);
        let r2 = store
            .upsert_request("xmpp", "user123", Some(serde_json::json!({"x": 1})))
            .unwrap();
        assert!(!r2.created);
    ...
    fn test_approve_adds_to_allow_from() {
        let (store, _) = test_store();
        let r = store.upsert_request("xmpp", "user456", None).unwrap();
        assert!(r.created);

        let approved = store.approve("xmpp", &r.code).unwrap();
        assert!(approved.is_some());
        assert_eq!(approved.unwrap().id, "user456");

        let allow = store.read_allow_from("xmpp").unwrap();
    ...
    fn test_approve_case_insensitive_code() {
        ...
        let approved = store.approve("xmpp", &code_lower).unwrap();
        assert!(approved.is_some());
    }
    ...
    fn test_approve_invalid_code_returns_none() {
        let (store, _) = test_store();
        store.upsert_request("xmpp", "user123", None).unwrap();
        let approved = store.approve("xmpp", "BADCODE1").unwrap();
    ...
    fn test_approve_rate_limited_after_many_failures() {
        ...
        for _ in 0..PAIRING_APPROVE_RATE_LIMIT {
            let _ = store.approve("xmpp", "WRONG01");
        }
        let err = store.approve("xmpp", "WRONG02").unwrap_err();
    ...
    fn test_is_sender_allowed_by_id() {
        let (store, _) = test_store();
        let r = store.upsert_request("xmpp", "user999", None).unwrap();
        store.approve("xmpp", &r.code).unwrap();

        assert!(store.is_sender_allowed("xmpp", "user999", None).unwrap());
        assert!(!store.is_sender_allowed("xmpp", "other", None).unwrap());
    }
    ...
    fn test_is_sender_allowed_by_username() {
        ...
        store
            ...
            .unwrap();
        let pending = store.list_pending("xmpp").unwrap();
        store.approve("xmpp", &pending[0].code).unwrap();

        // approve adds id to allow_from. For username we need to add it manually.
        // Actually approve adds entry.id which is "alice". So is_sender_allowed("xmpp", "alice", None) would work.
        assert!(store.is_sender_allowed("xmpp", "alice", None).unwrap());
        assert!(
            store
                .is_sender_allowed("xmpp", "alice", Some("alice"))
                .unwrap()
        );
    }
    ...
    fn test_channel_normalization() {
        let (store, _) = test_store();
        store.upsert_request("XMPP", "u1", None).unwrap();
        let pending = store.list_pending("xmpp").unwrap();
        assert_eq!(pending.len(), 1);
