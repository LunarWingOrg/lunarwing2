...
//! - Reading the gateway log file (`~/.lunarwing/gateway.log`)
...
/// Read the last N lines from `~/.lunarwing/gateway.log`.
///
        ...
        .map_err(|e| {
            anyhow::anyhow!(
                "Failed to connect to gateway at {url}: {e}\n\
                 Is the gateway running? Try `lunarwing gateway status`."
            )
        })?;
...
/// `None` on failure so that `lunarwing logs` works without any config.
