    ...
    pub mod agent {
        ...
        pub mod host {
            ...
            pub struct SshResponse {
                /// Process exit code (or -1 if the remote side reported none).
                pub exit_code: i32,
                /// Captured stdout bytes.
                pub stdout: _rt::Vec<u8>,
                /// Captured stderr bytes.
                pub stderr: _rt::Vec<u8>,
                /// True if stdout or stderr hit the host capture cap and was truncated.
                pub truncated: bool,
            }
            impl ::core::fmt::Debug for SshResponse {
                fn fmt(
                    &self,
                    f: &mut ::core::fmt::Formatter<'_>,
                ) -> ::core::fmt::Result {
                    f.debug_struct("SshResponse")
            ...
            #[allow(unused_unsafe, clippy::all)]
            /// Run a command on a preconfigured SSH host (if capability granted).
            ///
            /// Security:
            /// - `host` must be a known alias resolvable by the host's SSHBridge and
            ///   present in this tool's ssh allowlist; unlisted hosts are rejected.
            /// - Keys, the ssh-agent, and host-key verification stay host-side; the
            ///   guest only sends {host, command} and never sees key material.
            /// - stdout/stderr are scanned for leaked secrets before returning.
            /// - Bounded by the host's per-host connect/operation timeouts.
            ///
            /// Returns Err with a message if the host is not allowed, no key is stored,
            /// authentication or host-key verification fails, or a secret leak is
            /// detected in the output.
            pub fn ssh_exec(
                host: &str,
                command: &str,
            ) -> Result<SshResponse, _rt::String> {
                unsafe {
                    #[cfg_attr(target_pointer_width = "64", repr(align(8)))]
                    #[cfg_attr(target_pointer_width = "32", repr(align(4)))]
                    struct RetArea(
                        ...
                        0 => {
                            let e = {
                                ...
                                SshResponse {
                                    ...
                                    truncated: _rt::bool_lift(l12 as u8),
                                }
                            };
                            Ok(e)
                        }
...
A\x04\x01B\x1b\x01m\x05\x05trace\x05debug\x04info\x04warn\x05error\x04\0\x09log-\
level\x03\0\0\x01p}\x01r\x03\x06status{\x0cheaders-jsons\x04body\x02\x04\0\x0dht\
tp-response\x03\0\x03\x01r\x04\x09exit-codez\x06stdout\x02\x06stderr\x02\x09trun\
cated\x7f\x04\0\x0cssh-response\x03\0\x05\x01@\x02\x05level\x01\x07messages\x01\0\
\x04\0\x03log\x01\x07\x01@\0\0w\x04\0\x0anow-millis\x01\x08\x01ks\x01@\x01\x04pa\
ths\0\x09\x04\0\x0eworkspace-read\x01\x0a\x01k\x02\x01ky\x01j\x01\x04\x01s\x01@\x05\
\x06methods\x03urls\x0cheaders-jsons\x04body\x0b\x0atimeout-ms\x0c\0\x0d\x04\0\x0c\
http-request\x01\x0e\x01j\x01\x06\x01s\x01@\x02\x04hosts\x07commands\0\x0f\x04\0\
\x08ssh-exec\x01\x10\x01j\x01s\x01s\x01@\x02\x05aliass\x0bparams-jsons\0\x11\x04\
\0\x0btool-invoke\x01\x12\x01@\x01\x04names\0\x7f\x04\0\x0dsecret-exists\x01\x13\
\x03\0\x1alunarwing:agent/host@0.3.0\x05\0\x01B\x0a\x01ks\x01r\x02\x06paramss\x07\
context\0\x04\0\x07request\x03\0\x01\x01r\x02\x06output\0\x05error\0\x04\0\x08re\
sponse\x03\0\x03\x01@\x01\x03req\x02\0\x04\x04\0\x07execute\x01\x05\x01@\0\0s\x04\
\0\x06schema\x01\x06\x04\0\x0bdescription\x01\x06\x04\0\x1alunarwing:agent/tool@\
0.3.0\x05\x01\x04\0$lunarwing:agent/sandboxed-tool@0.3.0\x04\0\x0b\x14\x01\0\x0e\
sandboxed-tool\x03\0\0\0G\x09producers\x01\x0cprocessed-by\x02\x0dwit-component\x07\
