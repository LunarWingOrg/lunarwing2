...
use lunarwing::db::DatabaseHandles;
use lunarwing::secrets::{CreateSecretParams, SecretsCrypto, SecretsStore};
...
#[cfg(feature = "libsql")]
fn libsql_config(path: &std::path::Path) -> lunarwing::config::DatabaseConfig {
    lunarwing::config::DatabaseConfig {
        backend: lunarwing::config::DatabaseBackend::LibSql,
        url: secrecy::SecretString::from(String::new()),
        pool_size: 1,
        ssl_mode: lunarwing::config::SslMode::Prefer,
...
fn test_crypto() -> Arc<SecretsCrypto> {
    let key = secrecy::SecretString::from(lunarwing::secrets::keychain::generate_master_key_hex());
...
async fn connect_with_handles_returns_db_and_libsql_handle() {
    ...
    let (db, handles) = lunarwing::db::connect_with_handles(&config)
        .await
...
async fn connect_from_config_produces_working_db() {
    ...
    let db = lunarwing::db::connect_from_config(&config)
        .await
...
async fn secrets_store_from_handles_round_trips() {
    ...
    let (_db, handles) = lunarwing::db::connect_with_handles(&config)
        .await
        .expect("connect");

    let crypto = test_crypto();
    let store = lunarwing::secrets::create_secrets_store(crypto, &handles)
...
async fn db_create_secrets_store_standalone_round_trips() {
    ...
    let store = lunarwing::db::create_secrets_store(&config, crypto)
        .await
...
async fn both_secrets_factories_produce_compatible_stores() {
    ...
    let (_db, handles) = lunarwing::db::connect_with_handles(&config)
        .await
        .expect("connect");
    let store_a = lunarwing::secrets::create_secrets_store(Arc::clone(&crypto), &handles)
        .expect("store from handles");

    // Factory 2: db::create_secrets_store (standalone)
    let store_b = lunarwing::db::create_secrets_store(&config, crypto)
        .await
...
async fn extension_manager_with_process_manager_constructs() {
    ...
    use lunarwing::secrets::InMemorySecretsStore;
    use lunarwing::tools::ToolRegistry;
    use lunarwing::tools::mcp::McpProcessManager;
    use lunarwing::tools::mcp::McpSessionManager;
