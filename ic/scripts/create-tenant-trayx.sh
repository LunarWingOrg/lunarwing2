#!/usr/bin/env bash
#
# create-tenant-trayx.sh — provision the NEW tenant "trayx" from the staging
...
#
# Because the source repo is on staging, trayx is correct from the start: its
...
#
# Run as root:   sudo ic/scripts/create-tenant-trayx.sh [--yes]
#
# Edit the CONFIG block below (the XMPP JID especially) before running, or
# override any value via the environment, e.g.  XMPP_JID=trayx@example.org sudo -E ...
set -euo pipefail

# ---- CONFIG (edit me) -------------------------------------------------------
TENANT="trayx"
XMPP_JID="${XMPP_JID:-trayx@xmpp.sobe.world}"   # <-- confirm your XMPP domain
XMPP_PASSWORD="${XMPP_PASSWORD:-}"              # blank -> mt-admin generates one
LLM_API_KEY="${LLM_API_KEY:-}"                 # blank -> defaults to token-trayx (TensorZero proxy)
GOTIFY_URL="${GOTIFY_URL:-}"                    # optional, e.g. https://gotify.example.com
DOCKER_GROUP="${DOCKER_GROUP:-false}"          # set true ONLY if trayx runs Docker sandbox/workers
...
say "  LLM API key:    $([[ -n "$LLM_API_KEY" ]] && echo '(provided)' || echo 'token-trayx default')"
