...
run_darkirc_key_helper() {
  ...
  case "$command_name" in
    rewrite-baseline|list|status|doctor|adopt|export-migration|stage-migration|import-migration|migration-ready|recover)
      darkirc_load_compatibility
    ...
    darkirc-contact)
      ...
      case "$contact_action" in
        ...
        validate-migration)
          ...
          exit 0
          ;;
        *) die "usage: darkirc-contact {list|status|doctor|adopt|export-migration|stage-migration|import-migration|migration-ready|recover|validate-migration} ..." ;;
      esac
      [[ $# -eq 0 ]] || die "unexpected DarkIRC contact argument: $1"
      if [[ "$contact_action" == list || "$contact_action" == status || "$contact_action" == doctor ]]; then
        ports_registry_require_readonly
      else
        ports_registry_init
      fi
