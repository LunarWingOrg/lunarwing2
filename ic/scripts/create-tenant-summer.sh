#!/usr/bin/env bash
#
# create-tenant-summer.sh — provision the NEW tenant "summer" from the staging
...
#
# Because the source repo is on staging, summer is correct from the start: its
...
#
# Run as root:   sudo ic/scripts/create-tenant-summer.sh [--yes]
#
# Edit the CONFIG block below (the XMPP JID especially) before running, or
# override any value via the environment, e.g.  XMPP_JID=summer@example.org sudo -E ...
set -euo pipefail

# ---- CONFIG (edit me) -------------------------------------------------------
TENANT="summer"
XMPP_JID="${XMPP_JID:-summer@xmpp.sobe.world}"   # <-- confirm your XMPP domain
XMPP_PASSWORD="${XMPP_PASSWORD:-}"              # blank -> mt-admin generates one
LLM_API_KEY="${LLM_API_KEY:-}"                 # blank -> defaults to token-summer (TensorZero proxy)
GOTIFY_URL="${GOTIFY_URL:-}"                    # optional, e.g. https://gotify.example.com
DOCKER_GROUP="${DOCKER_GROUP:-false}"          # set true ONLY if summer runs Docker sandbox/workers
...
say "  LLM API key:    $([[ -n "$LLM_API_KEY" ]] && echo '(provided)' || echo 'token-summer default')"
