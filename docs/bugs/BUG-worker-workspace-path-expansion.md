...
> **Status: PARTIALLY-FIXED (verified against HEAD 2026-07-12).** Structured
> worker project directories now normalize `~`; prompt text and the Pebble
> executor still pass user-supplied paths without that normalization.
...
- `opencode4lunarwing/scripts/workspace_path.ts:1-5` maps `~` and `~/...` to
...
- The pure self-check covers `~`, `~/a/b`, absolute, relative, and `~user`
  inputs (`opencode4lunarwing/scripts/path_expand_test.ts:3-11`).
- Nanocode has the same helper and structured-directory wiring
  (`lunarcode4lunarwing/scripts/workspace_path.ts:1-5`,
...
(`opencode4lunarwing/scripts/opencode_task_executor.ts:99-102`), so the helper
cannot rewrite paths generated later by the worker. Pebble likewise uses
...
present in the opencode image (`opencode4lunarwing/Dockerfile:159-160,198`).
...
Verification used source inspection, Git ancestry, and fresh passing runs of
both checked-in TypeScript self-checks (OpenCode and Nanocode). No worker
container or Cargo command was run. Runtime prompt behavior remains unverified.
