...
> **Status: FIXED (static verification against HEAD 2026-07-12; historical
> test-run counts are not re-run here).** This consolidates the former
...
1. `executor::loop_engine::tests::action_then_text` expected three visible
   ...
   non-empty internal transcript (`ic/crates/lunarwing_engine/src/executor/loop_engine.rs:600-627`).
2. `codeact_multi_step` now searches `internal_messages` for `x = 30`
   (`loop_engine.rs:862-888`).
3. `tool_intent_nudge_injected` now searches the internal transcript for the
   nudge (`loop_engine.rs:722-750`).
...
(`ic/src/agent/agent_loop.rs:401-403`). Both surviving harness call sites wrap
the agent correctly:

- `ic/tests/support/gateway_workflow_harness.rs:292-298`
- `ic/tests/support/test_rig.rs:817-827`

The old report also claimed a third fix at
`ic/tests/e2e_telegram_message_routing.rs:231`. That file is absent from the
current tree and reachable history, so the historical claim cannot be
independently verified. A repository-wide search found no remaining plain
`agent.run()` site. Treat the missing third test as retired/obsolete rather
than as evidence of a current compile failure.
