...
> **Status: PARTIALLY-FIXED (verified against HEAD 2026-07-12).** The stale
> `HttpEndpointConfig` import warning is gone; `rand_check` still ignores its
> probability argument and always returns `false`.
...
`lunarwing_weechat_wss/weechat_relay/src/lib.rs:43-50`; there is no unused
`HttpEndpointConfig` import in the current file.

## Open latent behavior bug

`rand_check(_probability: f64) -> bool` at
`lunarwing_weechat_wss/weechat_relay/src/lib.rs:1761-1770` unconditionally
