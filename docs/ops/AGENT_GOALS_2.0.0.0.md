...
> **NOTE:** Edit the dev autonomous loop routine to use this file. Ensure each checkbox is checked off in the corresponding branch before committing, pushing, and opening a PR.
>
> Items are listed in rough recommended order but are not strictly sequential unless noted.

---

- [x] **1. Replace version references.** Update all`1.2.0` references in code and docs to`2.0.0` (where applicable only of course).

- [x] **2. Create overlay docs directory.**`cd {this repo} && mkdir -p docs/ov/`

- [x] **3. Bump crate versions.** Update all crate versions from`1.1.9` to`2.0.0`.

- [x] **4. Verify no crates were missed.** Confirm every relevant LunarWing crate — including xmpp bridge, wasm channels, and related components — is at`2.0.0`, NOT`1.1.9` or`1.1.8`.

6. [ ] First, read the blog post: https://blog.lunarwing.org/2026/07/12/lunarwingv2-the-next-frontier-of-private-self-hosted-ai-agents/ - THEN: Analyze the features not yet included in this AGENT_GOALS_2.0.0.0.md and document all of them into a new document under docs/proposals please.

- [ ] **5. Drop legacy`ironclaw-agent-v1` subprotocol offer.** Delete the`SUBPROTOCOL_LEGACY` offer in`ic/src/orchestrator/external_worker.rs`.`git rm` the 1.1.9-only repo-root compat symlinks`ironclaw_weechat_wss` and`darkirc_channel_for_ironclaw`. Deployed tenants must have re-run mt-admin unit regen by then.

- [ ] **6. LunarWing Web UI performance overhaul.** Address lag, button animation issues, and other UI problems.

- [x] **7. LunarWing Web MT admin setup integration.** Part 2 of an earlier plan discussed some time ago. Work started — see`lunarwing_mt_onboard_web/`. Being worked on by another agent or human.

- [ ] **8. Document open PRs and Issues.** Write a short doc with details of currently open PRs and Issues. Save to`docs/ops/`.

- [ ] **9. Audit cargo crate update status.** Inspect current crate versions and create a documented report of any crates that might still need updating. Verify whether the info dump below is still correct, then write up a document in`docs/ops/` detailing the status: is each piece verifiable? What outstanding issues remain? Which points have already been addressed?
...
- [ ] **10. Run full cargo test suite.** Full`--all-features --no-fail-fast` run. Expected baseline: lib 4087 passed / 0 failed / 4 ignored; all integration binaries + doctests pass except the 6 known-deferred (4`multi_tenant_system_prompt` architectural, 2`e2e_advanced_traces` bootstrap-greeting). See`docs/proposals/CARGO_TESTS_FIX.md`.

- [ ] **11. Fix remaining broken cargo tests.** Ensure updated documentation; create or rewrite new tests if necessary, then re-run cargo tests to ensure. Status: Fixed the 3 stale`lib` failures this cycle —`registry::embedded::tests::test_load_embedded_parses` (github→ssh sentinel),`cli::tests::test_help_output` +`test_long_help_output` (accepted rebranded insta snapshots). Lib re-run: 4087 passed / 0 failed. The 6 remaining failures are documented known-deferred (architectural / harness), not regressions.

- [ ] **12. Verify Gentoo WeeChat relay-api issue.** Document the accuracy of the issue below and write up a document in`docs/ops/` detailing the status: is each piece verifiable? What outstanding issues remain? Which points have already been addressed?

  <details>
  <summary><b>INFO DUMP — Gentoo WeeChat relay-api</b></summary>

  Issue: -relay-api is disabled (the API protocol needs cJSON).
...
- [ ] **13. Verify WeeChat multi-tenant ops guide accuracy.** Verify if the information below is accurate, then write up a document in`docs/ops/` detailing the status: is each piece verifiable? What outstanding issues remain? Which points have already been addressed?
...
- [ ] **14. Automate darkirc key exchange securely.** For this task, help plan it out — write up a doc in`docs/proposals/`.

- [ ] **15. Implement third-party memory routines (`memory_impl`).** Implement memory cleaning, de-duplication, and correction routines into the project. The user has created several advanced memory de-duplication routines on their own which are being used across three production agents (on 1.1.2). Goal is to either integrate these routines into LunarWing directly, or make it easy for new users to import them. More details may be added to this item at some point — currently a stub. For this task, help plan it out — write up a doc in`docs/proposals/`.

- [x] **16. Work on integrated testing routing with Christopher.** This will become very important. Do not resume the routine without permission.

- [ ] **17. MCP additions — next lifecycle/ops tranche.** Reference branch for hints (old failed implementation from previous opencode worker):`faility/failed-partial-old-item-3-20260711-0601`. Pick ONE from the recommended list below, implement it, check off this box.
 ...
 <details>
  <summary><b>Recommended List</b></summary>

  Recommended Next (pick ONE, implement it, check off item 17):

  - **17a. MCP deactivate/re-enable.** Stop the child, unregister its tools, and preserve configuration. Persist enabled = false so restart does not relaunch it. Add tool_deactivate, API, and web controls. This completes the lifecycle without involving WASM or workers.
  - **17b. Diagnostics and command preflight.** Extend doctor/status with transport, enabled state, and executable availability. Validate absolute commands or resolve commands through PATH. Report spawn and negotiation failures in the installed-extension response. Do not execute anything during installation.
  - **17c. Registry validation.** Add a validation test or registry validate command covering: exactly one of url or transport; valid stdio command/args/env; auth: none for stdio; duplicate names and unsupported transport types. This is almost entirely isolated to registry code and CI.
  - **17d. In-place configuration updates.** Let users edit command, args, env, or URL without remove/reinstall. If active, require explicit restart confirmation. Preserve existing registry precedence and approval rules.
  - **17e. Focused integration coverage.** Exercise the real install API through the router. Verify install → list → activate failure reporting → deactivate → remove. Add a browser-level check for HTTP/stdio mode switching and mobile layout.
...
- [ ] **18. Update`docs/architecture/` documents.** Review all docs under the architecture directory, update outdated documentation, and consolidate documents if possible.

- [ ] **19. Update`docs/bugs/` documents.** Review all docs under the bugs directory, update outdated documentation, and consolidate documents.

- [ ] **20. Update`docs/proposals/` documents.** Review all docs under the proposals directory, update outdated documentation, and consolidate documents if deemed necessary.

- [ ] **21. Update`docs/reviews/` documents.** Review all docs under the reviews directory, update outdated documentation, and consolidate documents.

- [ ] **22. Update`docs/guides/` documents.** Review all docs under the guides directory, update outdated documentation, and consolidate documents.
...
##### NOTE: All the markdown headers (`##`,`###`,`####`), tables, bold markers (`**`), and blockquote markers (`>`) have been stripped from inside the`<details>` sections. The content is now plain text with section labels written out as regular lines (e.g., "Architecture:", "Port Allocation:", "Troubleshooting — Adapter fails to connect..."). Code blocks are preserved since they contain actual commands and configs that need to be copy-pasted. The checklist items themselves (outside the`<details>` blocks) retain their checkbox formatting.