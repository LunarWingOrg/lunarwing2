...
- [LunarWing](https://github.com/LunarWingOrg/lunarwing) installed and running
...
This compiles the WASM and copies it plus `weechat.capabilities.json` to `~/.lunarwing/channels/` (or `$LUNARWING_BASE_DIR/channels/`).

---

## 3. Configure

Edit `~/.lunarwing/channels/weechat.capabilities.json` and set your relay password and allowed users:
...
**`dm_policy`** options:
- `"pairing"` — unknown users get a pairing code, approve with `lunarwing pairing approve weechat CODE`
...
```bash
lunarwing pairing approve weechat CODE123
lunarwing pairing list weechat    # see pending requests
```
...
- Run `lunarwing` with `RUST_LOG=debug` for verbose logs

**"HTTP not allowed" error:**
LunarWing's WASM sandbox may need to be patched to allow localhost HTTP.
